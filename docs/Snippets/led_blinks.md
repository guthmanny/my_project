...
```c
		...
		{    
			...
			if(envvol[0].db[0]>=-0.1f)  Dsp.dp->disp_mic1max = timset.t1s;
			if(envvol[1].db[0]>=-0.1f)  Dsp.dp->disp_mic2max = timset.t1s;
		}
...
```c
		...
		void setled_InVu(int vu1,int vu2,int vu1max,int vu2max)
		{
			...
			if(vu1max>0) setled_ledon_val(Mic1VuLedTable[4],0xff);   
